...
const producersSchema = z.array(producerSchema)

export function useProducers() {
  const [producers, setProducers] = useState<z.infer<typeof producerSchema>[]>([])
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/producers`, {
      ...
      .then(async (res) => {
        ...
        const parsed = producersSchema.parse(payload)

        setProducers(parsed)
      })
      .catch((err) => {
        console.error('❌ Erro ao buscar produtores:', err)
        setProducers([]) // fallback seguro
      })
      .finally(() => {
        setLoading(false)
      })
